...
\newcommand{\Neural}[1]{\textsf{Neural}\{#1\}}
...
\item $\InvSoftPlus(\beta_{l}) \sim \Dist{Normal}(\gst(1/5),0.1)$ % transmission rate (# of people infected infects per time unit, or more precisely, fraction of population infected by a person times scaling rate ) (?)
...
\item $\Evolve{s_l} = -\SoftPlus( \beta_{l} ) \cdot i_l \cdot s_l \cdot \frac{1}{N} + \xi \cdot r_l$ 
\item $\Evolve{e_l} = \SoftPlus( \beta_{l} ) \cdot  i_l \cdot  s_l \cdot \frac{1}{N} - \sigma \cdot  e_l$
\item $\Evolve{i_l} = \sigma \cdot e_l - \gamma \cdot  i_l$ % if set too small, values clipped
\item $\Evolve{r_l} = \gamma \cdot  i_l - \xi \cdot r_l$
