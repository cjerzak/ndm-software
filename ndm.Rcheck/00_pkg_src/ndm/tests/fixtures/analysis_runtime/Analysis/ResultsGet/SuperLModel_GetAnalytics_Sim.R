...
if(SimMode == T){
  ...
  if(ModelType != "DecoderOnly"){
    ...
    KLDiv_init <- median( rowSums(  np$array(batch_l$init_true) * (log(init_hat) - log(np$array(batch_l$init_true)) ) ))
    ...
    AbsDiff_gamma <- median(  abs( c(np$array(batch_l$gamma_true)) - gamma_hat) )
    RelAbsDiff_gamma <- median(  abs( c(np$array(batch_l$gamma_true)) - gamma_hat)/c(np$array(batch_l$gamma_true )))

    AbsDiff_sigma <- median(  abs( c(np$array(batch_l$sigma_true)) - sigma_hat) )
    RelAbsDiff_sigma <- median(  abs( c(np$array(batch_l$sigma_true)) - sigma_hat)/c(np$array(batch_l$sigma_true )))
  ...
  PolicyList_Model_natural <- list(
    # policy indicator
    "ScenarioIndicatorBasis" =  jnp$array( rep(0,nTimesTotal+0) ),

    # scenario (defines the smooth shift trajectory in the policy from the previous state)
    "Scenario" = jnp$array(InterpolatorBasis_[-c(1:ShiftFactor)][1:(nTimesTotal+0)])
  )

  PolicyList_Model_unnatural <- list(
    # policy indicator
    "ScenarioIndicatorBasis" = jnp$array( IndicatorBasis_un[-c(1:ShiftFactor)][1:(nTimesTotal+0)] ),
